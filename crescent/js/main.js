	   ...
	   $('.card-body').scrollTop($('.card-body')[0].scrollHeight);
		 ...
		 currSubActive.on("child_added", snap => {
		   ...
		   var currActive = firebase.database().ref().child('Users/' + email + '/CurrentActiveTab');
			...
			currActive.on("child_added", snap => {
				...
				var activeTab = snap.child("Active").val();
				var activeContent = snap.child("Content").val();
				var sidebutton = snap.child("Button").val();
			...
			CvShipRef.on("child_added", snap => {
				...
				$("#cv-shipping-icon").addClass("faa-shake animated");
				

				//document.getElementById("CvShipCountText").innerHTML = cvShipCount ;
				alerticon = document.getElementById('cv-shipping-icon');
				alerticon.setAttribute('data-badge', cvShipCount);
			});
		//end
		...
		//read CG Global
			...
			CgShipRef.on("child_added", snap => {
				...
				cgShipCount +=1;
				$("#cg-shipping-icon").addClass("faa-shake animated");
				//document.getElementById("CgShipCountText").innerHTML = cgShipCount ;
				alerticon = document.getElementById('cg-shipping-icon');
				alerticon.setAttribute('data-badge', cgShipCount);
			});
...
{ 
	...
	function updateCvLocal(){
		...
		//display cv
			//read cv filter
			$("#cv_body").empty();
			//$("#updateRemarksdirectoryText").empty();
			//$("#updatedirectoryText").empty();
			//$("#deletedirectoryText").empty();
			 var cvfilterRef =  firebase.database().ref().child('Users/' + email + '/Cv_Filter');
		...
		setTimeout(updateCvLocal, 5000);
	}
	
	
	
	function readCgLocal(){
